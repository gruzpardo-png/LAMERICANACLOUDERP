...
def production_import_csv():
    ...
    if not file:
        flash("Debes seleccionar un CSV.", "error")
        return redirect(url_for("production_view"))
    text = file.read().decode("utf-8-sig", errors="replace")
    items = parse_legacy_history_text(text)
    inserted = 0
    skipped = 0
    for item in items:
        ...
        inserted += 1
    db.session.add(AuditLog(actor=current_user().username, action="import_csv", detail=f"Insertados {inserted}, omitidos {skipped}"))
    db.session.commit()
    flash(f"CSV importado. Nuevos: {inserted}. Omitidos: {skipped}.", "success")
    return redirect(url_for("production_view"))
...
def users_view():
    if request.method == "POST":
        username = request.form.get("username", "").strip().lower()
        if not username:
            flash("Usuario requerido.", "error")
            return redirect(url_for("users_view"))
        user = User.query.filter_by(username=username).first() or User(username=username, password_hash=generate_password_hash(secrets.token_urlsafe(8)))
        ...
        db.session.add(user)
        db.session.add(AuditLog(actor=current_user().username, action="save_user", detail=username))
        db.session.commit()
        flash("Usuario guardado.", "success")
        return redirect(url_for("users_view"))
    users = User.query.order_by(User.username.asc()).all()
    return render_template("users.html", users=users, permissions=DEFAULT_PERMISSIONS)
